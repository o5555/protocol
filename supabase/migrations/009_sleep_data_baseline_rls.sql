-- Update RLS policy to allow challenge participants to see 7-day baseline data before challenge start
...
CREATE POLICY "Challenge participants can view shared sleep data" ON sleep_data
  FOR SELECT USING (
    EXISTS (
      ...
      AND sleep_data.date BETWEEN (c.start_date - INTERVAL '7 days')::date AND c.end_date
    )
  );
